...
public class UpdateChecker {
    ...
    private static final String CURRENT_VERSION = "1.0.0"; // Debe coincidir con la versión de tu instalador
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                if (conn.getResponseCode() == 200) {
                    try (JsonReader jsonReader = new JsonReader(new InputStreamReader(conn.getInputStream()))) {
                        jsonReader.setLenient(true); // Permite JSON malformado o respuestas laxas
                        JsonElement element = new Gson().fromJson(jsonReader, JsonElement.class);

                        if (element != null && element.isJsonObject()) {
                            JsonObject response = element.getAsJsonObject();
                            if (response.has("version") && response.has("url")) {
