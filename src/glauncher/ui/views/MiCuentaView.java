...
public class MiCuentaView {
    ...
    private Node createSkinsView() {
        ...
        CheckBox chkSlimModel = new CheckBox("Modelo Slim (Alex)");
        chkSlimModel.setStyle("-fx-text-fill: white; -fx-cursor: hand;");
        ...
        sidebar.getChildren().addAll(lblSaved, savedSkins, chkSlimModel);
        ...
        Button btnApply = new Button("Aplicar al Juego");
        btnApply.setTooltip(new javafx.scene.control.Tooltip("Establece esta skin como la skin activa para el juego."));
        btnApply.setStyle("-fx-background-color: #28a745; -fx-text-fill: white; -fx-cursor: hand; -fx-font-weight: bold; -fx-background-radius: 20;");
        ...
        floatingBar.getChildren().addAll(btnUpload, btnApply, new javafx.scene.control.Separator(javafx.geometry.Orientation.VERTICAL), btnReset);
        ...
        btnUpload.setOnAction(e -> {
            ...
            if (file != null) {
                ...
                btnApply.setUserData(file); // Guardar el archivo para aplicarlo
                MainView.showNotification("Skin Cargada", "La textura se ha aplicado correctamente.", "success");
            }
        });

        btnApply.setOnAction(e -> {
            File skinFile = (File) btnApply.getUserData();
            if (skinFile != null && skinFile.exists()) {
                saveActiveSkin(skinFile.getAbsolutePath(), chkSlimModel.isSelected() ? "slim" : "default");
            } else {
                MainView.showNotification("Error", "Primero debes subir una skin válida.", "error");
            }
        });
    ...
    private void saveActiveSkin(String skinPath, String skinType) {
        try {
            JsonObject session = new JsonObject();
            if (SESSION_FILE.exists()) {
                try (FileReader reader = new FileReader(SESSION_FILE)) {
            ...
            session.addProperty("custom_skin_path", skinPath);
            session.addProperty("custom_skin_type", skinType);
            
            try (FileWriter writer = new FileWriter(SESSION_FILE)) {
                gson.toJson(session, writer);
            }
            MainView.showNotification("Skin Aplicada", "La skin se usará en el próximo lanzamiento.", "success");
        } catch (Exception e) { e.printStackTrace(); }
    }
